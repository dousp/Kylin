...
import com.dou.kylin.security.shiro.common.BaseLogger;
...
public class ShiroRedisCache<K, V> extends BaseLogger implements Cache<K, V> {
    ...
    public ShiroRedisCache(String name, ICache cache) {
        this.name = name;
        this.cache = cache;
    }
    ...
    public V get(K key) throws CacheException {
        ...
        try {
            if (key == null) {
                ...
                V value = (V) cache.getHashCached(getByteName(), getByteKey(key));
                return value;
            }
        } catch (Throwable t) {
    ...
    public V put(K key, V value) throws CacheException {
        ...
        try {
            cache.updateHashCached(getByteName(), getByteKey(key), SerializeUtil.serialize(value), null);
            return value;
        } catch (Throwable t) {
            throw new CacheException(t);
        }
